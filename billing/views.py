...
def invoice_detail(request, invoice_id):   
    ...
    buttons_list = [
        ...
        {'url': reverse('billing:add_ipd_consultation_bill', args=[invoice_id]), 'label': 'Add IPD Consultation Bill', 'btn_class': 'btn-warning'},
        ...
        {'url': reverse('billing:add_ward_bill', args=[invoice_id]), 'label': 'Add Ward bill', 'btn_class': 'btn-success'},
        ...
        with transaction.atomic():
            ...
            if not log:
                try:
                    ...
                    messages.warning(request, f"Doctor service rate not defined for {doctor}")
...
def ot_booking(request, invoice_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ot_bill.total_charge = float(ot_bill.operation_theatre.hourly_rate or 0) * float(duration)
            ot_bill.save()
    ...
    return render(request, 'billing/add_ot_bill.html', {
        'form': form,
        'invoice': invoice
    })
...
def add_doctor_payment(request,doctor_id):
    ...
    if total_due_amount<= 0: 
        messages.warning(request,'This doctor has no due payment, Please check payment status again')
